...
const actionLabel: Record<string, string> = {
  ...
  "RESUBMIT": "ส่งแบบฟอร์มฝึกงานใหม่อีกครั้ง",
      ...
      <div className="max-w-6xl mx-auto py-8 px-4 flex flex-col gap-6 font-sans">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col gap-8 animate-fade-up delay-0">
          <div className="flex justify-between items-start">
            ...
            <Link href="/intern/internship" className="px-6 py-3 bg-gradient-to-r from-[#FF9B5C] to-[#F16422] text-white font-semibold rounded-xl shadow-md hover:shadow-lg hover:brightness-95 flex items-center gap-2 transition-all duration-150 active:scale-[0.97]">
          ...
          {isLoading ? (
            ...
            <div className="bg-[#f8f9fa] rounded-xl p-6 border border-gray-100 flex flex-col gap-4">
              <div className="flex justify-between items-end">
                ...
                {!isCompleted && <p className="text-sm font-bold text-[#5e2b97]">เหลืออีก {totalDays - daysCompleted} วัน</p>}
              </div>
        ...
        {isRejected && (
          <div className="bg-red-50 border-2 border-red-100 rounded-2xl p-6 flex items-start gap-4 animate-fade-up delay-100">
            ...
            <div className="flex-1">
              ...
              <div className="mt-5">
                <Link href="/intern/internship" className="px-5 py-2.5 bg-red-600 text-white text-sm font-bold rounded-xl hover:bg-red-700 transition-all shadow-md shadow-red-200 flex items-center gap-2 w-fit active:scale-95">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          ...
          <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col hover:-translate-y-0.5 hover:shadow-md transition-all duration-300 ease-out animate-fade-up delay-100 h-full">
              ...
              <div className="flex flex-col gap-4 text-sm flex-1">
                ...
                <div><p className="text-gray-400 text-xs tracking-wider mb-1">เบอร์โทรศัพท์</p><p className="font-semibold text-gray-900">{profile.phone || "N/A"}</p></div>
                <div><p className="text-gray-400 text-xs tracking-wider mb-1">เบอร์ติดต่อฉุกเฉิน</p><p className="font-semibold text-red-600">{profile.parentPhone || "N/A"} (ผู้ปกครอง)</p></div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col hover:-translate-y-0.5 hover:shadow-md transition-all duration-300 ease-out animate-fade-up delay-150 h-full">
              ...
              <div className="border border-gray-200 rounded-xl p-2 bg-gray-50/20">
                <div className="flex flex-col gap-2 max-h-[220px] overflow-y-auto pr-1 custom-scrollbar">
                  {profile.files && profile.files.length > 0 ? profile.files.map((file) => {
                    ...
                    const isClickable = displayStatus === "APPROVED" || displayStatus === "REJECTED";
                    const tooltip = displayStatus === "APPROVED" ? "คลิกเพื่อแก้ไขไฟล์" : displayStatus === "REJECTED" ? "คลิกเพื่ออัปโหลดใหม่" : "";
                    ...
                    return (
                      <div
                        ...
                        className={`flex justify-between items-center bg-white border border-gray-100 rounded-lg p-2.5 transition-all ${isClickable
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col hover:-translate-y-0.5 hover:shadow-md transition-all duration-300 ease-out animate-fade-up delay-150 h-full">
              ...
              <div className="flex flex-col gap-4 text-sm flex-1">
                ...
                <div><p className="text-gray-400 text-xs tracking-wider mb-1">ระดับการศึกษา</p><p className="font-semibold text-[#5e2b97]">{profile.educationLevel || "N/A"}</p></div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col h-full hover:-translate-y-0.5 hover:shadow-md transition-all duration-300 ease-out animate-fade-up delay-200">
            <div className="flex items-center gap-2 mb-5">
              <div className="p-2 bg-purple-50 rounded-lg text-[#5e2b97]">
            ...
            <div className="relative border-l-[3px] border-gray-100 ml-2 mt-2 flex flex-col gap-0 pb-2">
              {recentActivities.length > 0 ? recentActivities.map((act, idx: number) => {
                const isReject = act.action.includes("REJECT") || act.action.includes("DELETE");
                const isUpdate = act.action.includes("UPDATE");
                const dotColor = isReject ? "bg-red-500 border-red-200 outline-red-100" : isUpdate ? "bg-yellow-500 border-yellow-200 outline-yellow-100" : "bg-emerald-500 border-emerald-200 outline-emerald-100";
                const statusChange = act.action === "STATUS_CHANGED" ? parseStatusChangedDescription(act.description) : null;
                return (
                  <div
                    key={act.id}
                  ...
                  >
                    <div className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-4 border-white outline outline-4 ${dotColor} z-10 transition-transform group-hover:scale-110`} />
                    <p className="text-sm font-bold text-gray-800 leading-snug">{actionLabel[act.action] || act.action}</p>
                    {statusChange ? (
                      <div className="mt-0.5 flex flex-col gap-1">
                        <p className="text-xs text-gray-500">
                          <span>แอดมินเปลี่ยนสถานะเป็น </span>
                          <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${statusColor[statusChange.status] ?? "bg-gray-100 text-gray-600"}`}>
                            {statusLabel[statusChange.status] ?? statusChange.status}
                          </span>
                        </p>
                        {statusChange.note && (
                          <p className="text-xs text-gray-500">หมายเหตุ: {statusChange.note}</p>
                        )}
                      </div>
                    ) : (
                      <p className="text-xs text-gray-500 mt-0.5">{act.description}</p>
                    )}
                    <p className="text-[11px] text-gray-400 mt-1">{dayjs(act.createdAt).fromNow()}</p>
                  </div>
                );
              }) : (
