...
describe("admin actions", () => {
  ...
  describe("updateInternshipStatus", () => {
    ...
    it("triggers email when status is EDIT_REQUESTED", async () => {
      (getSession as any).mockResolvedValue({ user: { email: "admin@test.com" } });
      (prisma.user.findUnique as any).mockResolvedValue({ id: "admin-id" });
      (prisma.internship.findUnique as any)
        .mockResolvedValueOnce({ id: "id", status: "PENDING", studentId: "student-id" })
        .mockResolvedValueOnce({ 
          id: "id",
          student: { email: "student@test.com", studentProfile: { firstNameTh: "John", lastNameTh: "Doe", prefix: "Mr." } }
        });
      (prisma.internship.update as any).mockResolvedValue({ studentId: "student-id" });
      (resend.emails.send as any).mockResolvedValue({ data: { id: "email-id" } });
      
      process.env.ADMIN_NOTIFICATION_EMAIL = "admin-notify@test.com";
    ...
    it("handles error in email logic but still returns success", async () => {
      (getSession as any).mockResolvedValue({ user: { email: "admin@test.com" } });
      (prisma.user.findUnique as any).mockResolvedValue({ id: "admin-id" });
      ...
      (prisma.internship.update as any).mockResolvedValue({ studentId: "student-id" });
      (resend.emails.send as any).mockRejectedValue(new Error("Email Fail"));
    ...
    it("skips email if student profile is missing", async () => {
      (getSession as any).mockResolvedValue({ user: { email: "admin@test.com" } });
      (prisma.user.findUnique as any).mockResolvedValue({ id: "admin-id" });
      (prisma.internship.findUnique as any).mockResolvedValue({ 
        id: "id", 
    ...
    it("handles missing optional student info in email logic", async () => {
      (getSession as any).mockResolvedValue({ user: { email: "admin@test.com" } });
      (prisma.user.findUnique as any).mockResolvedValue({ id: "admin-id" });
      (prisma.internship.findUnique as any).mockResolvedValue({ 
        id: "id", 
