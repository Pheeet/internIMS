...
export async function updateInternshipStatus(
  ...
  try {
    ...
    await prisma.auditLog.create({
      ...
      // Send email if marking as EDIT_REQUESTED
      if (newStatus === "EDIT_REQUESTED") {
        try {
          console.log("[NOTIFY][admin] trigger", {
            internshipId,
            newStatus,
            remarks: remarks || null,
          });

          const internshipWithProfile = await prisma.internship.findUnique({
            where: { id: internshipId },
            include: {
              student: {
          ...
          if (internshipWithProfile?.student?.studentProfile) {
            const studentProfile = internshipWithProfile.student.studentProfile;
            const adminEmail = process.env.ADMIN_NOTIFICATION_EMAIL;
            const fromEmail = process.env.RESEND_FROM_EMAIL || "onboarding@resend.dev";
            const studentEmail = internshipWithProfile.student.email;

            console.log("[NOTIFY][admin] env", {
            ...
            if (adminEmail) {
              ...
              const position = internshipWithProfile.position || "ไม่ระบุ";
              const company = internshipWithProfile.company || internshipWithProfile.department || "ไม่ระบุ";
              const flaggedFields = internshipWithProfile.flaggedFields as string[] | undefined;

              console.log("[NOTIFY][admin] sending edit-requested email", {
                internshipId,
                to: adminEmail,
                studentEmail,
                flaggedFieldsCount: Array.isArray(flaggedFields) ? flaggedFields.length : 0,
              });

              const resendResult = await resend.emails.send({
                from: fromEmail,
                to: adminEmail,
                subject: "นักศึกษาได้รับแจ้งให้แก้ไขข้อมูลฝึกงาน - การฝึกงาน",
              ...
              console.log("[NOTIFY][admin] resend success (edit-requested)", {
                internshipId,
                resendId: resendResult?.data?.id,
                resendError: resendResult?.error || null,
              });
              console.log("[DEBUG] Change requested email sent for internship:", internshipId);
            } else {
              console.warn("[NOTIFY][admin] skipped: ADMIN_NOTIFICATION_EMAIL not configured");
            }
          } else {
            console.warn("[NOTIFY][admin] skipped: internship/student profile not found", { internshipId });
          }
        } catch (emailError) {
          console.error("[DEBUG] Failed to send change-requested email:", emailError);
    ...
    await prisma.$transaction([
      ...
      prisma.auditLog.create({
        data: {
          ...
          newValue: "APPROVED",
          description: "แอดมินส่งกลับแก้ไข (ย้อนข้อมูลจาก snapshot) และเปลี่ยนสถานะกลับเป็น APPROVED",
        },
      }),
    ]);
