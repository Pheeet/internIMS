...
export default function ApplicationReviewDrawer({
  ...
  const isAllApproved =
    files.length > 0 &&
    files.every((f) => fileReviews[f.id]?.status === "APPROVED");
  ...
  const handleSubmitReview = () => {
    ...
    startTransition(async () => {
      // Save all file review statuses first
      await Promise.all(
        Object.entries(fileReviews)
          .filter(([, v]) => v.status === "REJECTED" || v.status === "APPROVED")
          .map(([id, v]) =>
            fetch(`/api/admin/attachments/${id}/status`, {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ status: v.status, rejectReason: v.reason || null }),
            })
          )
      );
      await updateInternshipStatus(internship.id, "APPROVED", "");
      onClose();
    });
  };
                                ...
                                <>
                                  <button
                                    ...
                                    onClick={() => {
                                      setFileReviews((prev) => ({
                                        ...prev,
                                        [file.id]: { ...prev[file.id], status: "REJECTED" },
                                      }));
                                      fetch(`/api/admin/attachments/${file.id}/status`, {
                                        method: "PATCH",
                                        headers: { "Content-Type": "application/json" },
                                        body: JSON.stringify({ status: "REJECTED", rejectReason: fileReviews[file.id]?.reason || null }),
                                      });
                                    }}
                                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-bold transition-all ${isRejected
                                        ? "bg-red-500 text-white border border-red-500"
                                      : isApproved
                                          ? "border border-red-200 text-red-300 cursor-not-allowed opacity-50"
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      setFileReviews((prev) => ({
                                        ...prev,
                                        [file.id]: { status: "APPROVED", reason: "" },
                                      }));
                                      fetch(`/api/admin/attachments/${file.id}/status`, {
                                        method: "PATCH",
                                        headers: { "Content-Type": "application/json" },
                                    ...
                                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-bold transition-all ${isApproved
                                      ...
                                      : isRejected
                                          ? "border border-green-200 text-green-300 cursor-not-allowed opacity-50"
