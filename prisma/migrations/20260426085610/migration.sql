/*
  Warnings:

  - The values [PENDING_UPDATE] on the enum `InternshipStatus` will be removed. If these variants are still used in the database, this will fail.

*/
-- CreateEnum
CREATE TYPE "AttachmentStatus" AS ENUM ('PENDING', 'APPROVED', 'REJECTED');

-- AlterEnum
BEGIN;
CREATE TYPE "InternshipStatus_new" AS ENUM ('PENDING', 'REJECTED', 'EDIT_REQUESTED', 'APPROVED', 'COMPLETED');
ALTER TABLE "public"."Internship" ALTER COLUMN "status" DROP DEFAULT;
ALTER TABLE "Internship" ALTER COLUMN "status" TYPE "InternshipStatus_new" USING ("status"::text::"InternshipStatus_new");
ALTER TYPE "InternshipStatus" RENAME TO "InternshipStatus_old";
ALTER TYPE "InternshipStatus_new" RENAME TO "InternshipStatus";
DROP TYPE "public"."InternshipStatus_old";
ALTER TABLE "Internship" ALTER COLUMN "status" SET DEFAULT 'PENDING';
COMMIT;

-- AlterTable
ALTER TABLE "Attachment" ADD COLUMN     "rejectReason" TEXT,
ADD COLUMN     "status" "AttachmentStatus" NOT NULL DEFAULT 'PENDING';

-- AlterTable
ALTER TABLE "StudentProfile" ALTER COLUMN "educationLevel" DROP NOT NULL,
ALTER COLUMN "educationLevel" DROP DEFAULT,
ALTER COLUMN "institution" DROP NOT NULL,
ALTER COLUMN "institution" DROP DEFAULT,
ALTER COLUMN "faculty" DROP NOT NULL,
ALTER COLUMN "faculty" DROP DEFAULT,
ALTER COLUMN "major" DROP NOT NULL,
ALTER COLUMN "major" DROP DEFAULT;
