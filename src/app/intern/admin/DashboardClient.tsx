...
type ToneColor = "green" | "red" | "yellow" | "sky" | "violet" | "orange" | "teal" | "amber";
...
const toneColorDotClasses: Record<ToneColor, string> = {
  ...
  yellow: "bg-yellow-500 border-yellow-200 outline-yellow-100",
  ...
  amber:  "bg-amber-600 border-amber-200 outline-amber-100",
};
          ...
          <div>
            ...
            <div className="relative border-l-[3px] border-gray-100 ml-4 mt-4 space-y-8 pb-4">
              ...
              {activities.map((act, idx: number) => {
                const dotColorClass = toneColorDotClasses[act.toneColor] || "bg-gray-400";
                const isStatusChanged = act.action === "STATUS_CHANGED" && act.oldValue && act.newValue;
                  ...
                  >
                    ...
                    {isStatusChanged ? (
                      <div className="flex items-center gap-1.5 flex-wrap mt-1">
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${act.oldValue && STATUS_LABELS[act.oldValue] ? STATUS_LABELS[act.oldValue].color : "text-gray-600 bg-gray-50"}`}>
                          {act.oldValue && STATUS_LABELS[act.oldValue] ? STATUS_LABELS[act.oldValue].label : act.oldValue}
                        </span>
                        <span className="text-gray-400 text-[10px]">→</span>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${act.newValue && STATUS_LABELS[act.newValue] ? STATUS_LABELS[act.newValue].color : "text-gray-600 bg-gray-50"}`}>
                          {act.newValue && STATUS_LABELS[act.newValue] ? STATUS_LABELS[act.newValue].label : act.newValue}
                        </span>
                        <span className="text-[11px] text-gray-500 ml-1">
                          โดย <span className="text-purple-400 font-medium">{act.actorName}</span>
                        </span>
                      </div>
                    ) : (
                      <>
